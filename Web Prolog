...
<p>This might be a better ranking: <a href="http://www.tiobe.com/index.php/content/paperinfo/tpci/index.html">http://www.tiobe.com/index.php/content/paperinfo/tpci/index.html</a></p>
...
<p>Of course, in this situation safety becomes very important. Just as JavaScript doesn’t allow you to access the file system of the client device, Web Prolog shouldn’t allow you to access the file system of the server.</p>
<hr>
...
<p>It is often a good idea to run components concurrently and use asynchronous message passing to enable them to communicate. It doesn’t really matter if components are implemented in Web Prolog or not, since a non-Prolog component can always be proxied by a pengine – a kind of adapter between a set-at-a-time cursor-limit based interface and a one-tuple-at-a-time interface if you want.</p>
...
<p>I would like to convince our NLP colleagues Johan Bos, Patrick Blackburn and Kristina Striegnitz that a “full stack” dialogue system can be built in a uniform language, taking care of things like parsing, generation, semantic interpretation, dialogue management, knowledge representation and reasoning, and, relying on the availability of JavaScript speech API<img draggable="false" class="emoji" alt="😒" src="https://twemoji.maxcdn.com/36x36/1f612.png"> in browsers, speech recognition and synthesis.</p>
<p>Interaction part written in JavaScript. Web Prolog for sophisticated interaction., controlled on the server side. Listening, thinking, talking…</p>
<p>“linguistic interaction needs intelligence”</p>
<p>The interaction demands intelligence, the ability to interact appropriately to speech acts given the context in question. Part of the context is obviously so called “world knowledge”, perhaps in a fairly closed domain, but still, something that we will not be able to store in the client.</p>
<p>Another thing is architecture. Sophisticated conversational agents. Galaxy Communicator. Scxml.</p>
<p>SCXML – referera till tyskarnas försök att använda Prolog</p>
...
<p>An interface built on Bootstrap <em>cards</em>, ideal for the conversation with a mobile device. The idea would be to use cards as the pengine’s contributions to the dialogue between the user and a running pengine. Not in the same way as SWISH uses its runners though, were each runner “card” corresponds to a pengine. Instead, each <em>system contribution</em>, of which there may be many in the conversation with a pengine, is given as a card. So <code>pengine_input/2</code> and <code>pengine_output/1</code> have roles to play here.</p>
<p>Card expert system<br>
Card interface to Markus’ tutoring system</p>
...
<p>Web Prolog, with it’s markedly rule based flavor is, as far as I can see, perfect for home automation. Maybe with one process per device, modeling external devices, sending incoming sensory data from Things as messages to be sent to parts of the system (other processes, some of them pengines) that decides how to react to them. The other way around, …The Web of Prolog becomes also a <em>Web of Things</em>.</p>
...
<p>Here’s a link to a tiny expert system example, just to demo the interaction with a Web browser:</p>
<p><a href="http://pengines.swi-prolog.org/apps/scratchpad/index.html#1c12c372-8714-11e5-be28-00163e8fc784.html">http://pengines.swi-prolog.org/apps/scratchpad/index.html#1c12c372-8714-11e5-be28-00163e8fc784.html</a></p>
...
<p>A domain-specific language (DSL) is a programming language (or specification language) of limited expressiveness focused on a particular application domain. Regular expressions and CSS are often used as examples of DSL<img draggable="false" class="emoji" alt="😒" src="https://twemoji.maxcdn.com/36x36/1f612.png">.</p>
...
<h3 id="spawning-sending-and-receiving">Spawning, sending and receiving</h3>
...
<p>The following example, borrowed from Fred Hébert’s excellent Erlang book, shows how an agent can be made to hold state and update it during a conversation.</p>
...
<p>This kind of mutable state you get in Web Prolog is exactly the same you get in Erlang, nothing less, nothing more, which is usually via processes and you need to send messages to change or read from the process state. There are other ways to implement mutable state in Prolog – by means of assert and retract for example, or by means of nb_set/2 and nb_get/2. One potentially important advantage of using a process as a mutable state is that it can be made distributed.</p>
...
<p>Here’s an <em>example</em> of what can be used to specialise the generic server:</p>
...
<p>When we program we want to structure the code into “difficult” and “easy” modules. The difficult modules should be few and written by expert programmers. The easy modules should be many and written by less experienced programmers.</p>
...
<p>The above are called OTP behaviors. Normally you would create a module and mark it as an implementation of one of these behaviors, then implement the callbacks (“hooks” in Prolog parlance) required by that behavior. This saves you from reinventing the wheel, but you need to familiarize yourself with what is required to implement each behavior and when to expect the OTP framework to call your callback functions.</p>
<h3 id="concurrency">Concurrency</h3>
<p>A we noted before, although concurrent programming with pengines is clearly possible, it hasn’t undergone any validation to speak of, and therefore this possibility is still underexploited. In contrast, Erlang has for a long time been used and therefore also been validated as a powerful tool for concurrent programming. Small, medium and large applications have been built, pitfalls have been located, tools and practices have been developed, and books and tutorials have been written. For this reason alone it is probably a good idea to borrow from Erlang as much as possible.</p>
<p>It is easy to run several processes in parallell in Erlang, and to allow them to communicate with each other. In the following example – given in both Erlang and Web Prolog – two processes are first created and then start sending messages to each other a specified number of times.</p>
<p><img src="https://i.imgur.com/vgRNt6V.png" alt="enter image description here"></p>
<h3 id="non-determinism">Non-determinism</h3>
<p>It is wise to entertain a suspicion of unexpected interactions between language features and possible impedance mismatches between the two paradigms – between Prolog’s relational, non-deterministic programming model and Erlang’s process and message passing model. Can we really avoid the Lego-Meccano situation in this case? How well do these Erlang-ish constructs mix with Prolog, with backtracking for example? As we shall see, one of the necessary tricks is to allow (the action part) of a receive to <em>fail</em> and, by failing, force backtracking.</p>
...
<p>This will work in the sense that it first will print ‘a’. It will then wait for a message coming from another process. If this message is ‘next’, backtracking will take place, ‘b’ will be generated and printed, and at this point the variable bindings will also be written. This demonstrates that Prolog’s backtracking behaviour is perfectly compatible with the proposed Erlang-like mechanism. However, if the message is ‘stop’, no more answers will be sent and the spawned process will terminate. The parent process will however not terminate, since the second receive will be waiting for a message to arrive.  Obviously, we will have to build something more generic.</p>
<p>Pengines is a software abstraction that can be built on top of <code>spawn/2</code>, <code>send/2</code> and <code>receive/2</code>. Browser Api, Parallell processing, Distributed processes, they all make use of <code>spawn/2</code>, <code>send/2</code> and <code>receive/2</code>. The browser is capable of using it too. Later we’ll see that we can build another abstraction on top of pengines, the RPC mechanism. Abstractions such as these can be compared to Erlang’s <em>behaviors</em>. They are difficult to build, but once they are built they can be fairly easily instantiated and tailored to specific tasks.</p>
...
<p>Something annoying with the pengines is that the programmer who’s going to use them has to know about the protocol that’s been invented for them. A good way to solve this is to abstract messages away – hiding them – with the help of predicates dealing with sending (and receiving?) them:</p>
...
true.
?- receive({M -&gt; writeln(M)}).
success(123@'http://ex.org#a’, q(a), true)
true.
...
<li>Michael A. Covington, Donald Nute, and André Vellino, Prolog Programming in Depth, Second edition, Prentice-Hall, 1997<br>
...
<li>Bart Demoen, Phuong-Lan Nguyen, Tom Schrijvers, Remko Tronçon, The First 10 Prolog Programming Contests<br>
...
<p>Different Web Prolog nodes may implement different “profiles” (fragments or a sublanguages) of the Web Prolog language, each of which achieves efficiency in a different way and is useful in different application scenarios.</p>
...
<p>My suggestion is that we instead implement the Erlang-ish primitives on top of <code>thread_*</code> predicates and the Websockets library, and that we build a new implementation (in the form of a library or as a built-in – I’m not sure what would be best here) in Web Prolog. Then the Prolog Web can be based on this implementation instead and other Erlang/OTP style <em>behaviours</em> can be built on top of Erlang-ish primitives + Prolog.</p>
<p>As have been demonstrated, it is possible to implement spawn, send and receive on top of Prolog’s thread predicates. It is not optimal since the underlying processes will be threads rather than the super lightweight processes of Erlang. This will change, not the semantics, but the <em>pragmatics</em> of the language – what you can do with it. Web Prolog can probably be implemented on top of Erlang too.</p>
<p>I don’t think it is possible to implement Web Prolog with HTTP as the transport layer, except possibly by using a greedy long polling strategy or something like that. With Websockets as the transport layer should be possible though.</p>
...
commercial users or -in some cases- merely a statement that “this<br>
feature would be of great help for my business”. Even without grants,<br>
...
you an idea.  I’m seeking for ways to get these things realised<br>
quicker than doing it myself next to other obligations.</p>
<ul>
<li>Tabling.  Most will know what it is …</li>
...
<li>Establish tutorial material aiming at SWI-Prolog’s extensions and<br>
libraries.</li>
<li>Improve the manual and website (get Anne’s work finished)</li>
<li>RDF support enhancements: faster reload, improve query optimization,<br>
notably for queries that involve literals.</li>
</ul>
<p>Some of these enhancements have already been implemented, and all of the remaining ones are perfecly in line with the development of Web Prolog. However, I think that instead of establishing “tutorial material aiming at SWI-Prolog’s extensions and libraries”, we should develop tutorial material aiming at Web Prolog. For one thing, it will be much easier, and will naturally include a fair number of SWI-Prolog’s extensions and libraries, but will be packaged as language features rather than libraries. A text book might be the best option here.</p>
<p>Jan shouldn’t have to do the JavaScript programming himself. The best would be if he concerned himself with the standard and with the implementation of e.g. tabling in SWI-Prolog. The Web Prolog community needs a couple of good front-end developers. We might want to recruit the on for example <a href="openhatch.org">openhatch.org</a>. The community around Elixir is a good example. We also need people to write a book introducing Web Prolog.</p>
...
<p>A good programming language standard makes writing portable programs possible – provided that the standard covers all the features used by the program, and that implementations conform to the standard.</p>
<p>The current ISO standard for Prolog has failed in the sense that few existing Prolog systems of today are conformant, but also due to the fact that most Prolog system go far beyond the standard in some respects. This has made porting Prolog programs somewhat difficult. SWI-Prolog is one of the systems that doesn’t really conform, a fact lamented by some people, witness the following quote from a mail message sent to the SWI-Prolog mailing list from Markus Triska:</p>
<blockquote>
...
<blockquote>
<p>The ISO core is great to have, but too small, too much bound by poor decisions from the past (e.g., numbers and text) and impossible to evolve. Ideally we’d have a more flexible and wider adopted standardisation effort. All attempts in that direction failed due to lack of commitment and time as well as a wide variation in the focus of the various systems. (Jan, in a mail message dated ??).</p>
</blockquote>
<p>Here’s my take on this: If we really need to conform to a standard in order to attract users “working in a regulated environment”, and if the current ISO standard is impossible to evolve, then is seems we need a <em>new</em> standard, incorporating things like unicode, proper strings and the SWI-Prolog dicts. This would eventually, at least in principle, satisfy the demand for a standard posed by lawyers and insurance companies, and at the same time allow us to evolve the Prolog language. For industrial users, the <em>prospect</em> of a future standard is likely to be enough to increase trust.</p>
...
<p>Also, the Web – our focus in this proposal – thrives on standards, and if we want maximal impact, we ought to present Web Prolog as a standard, but this time, as a Web standard.</p>
...
<p>The W3C reliance of tests suits SWI-Prolog. Jan’s comment to a mail message questioning this reliance:</p>
<blockquote>
...
<p>A W3C Community Group is an open forum, without fees, where Web developers and other stakeholders develop specifications, hold discussions, develop test suites, and connect with W3C’s international community of Web experts.</p>
<p><img src="https://i.imgur.com/95FUOIS.png" alt="enter image description here"></p>
<p>Interestingly, there is an already existing group that seems to partly fit our purposes (although it hasn’t been active at all):</p>
<blockquote>
...
<p>Comment: There is currently, as far as I know, no other programming language as well adapted to the processing of RDF as Prolog. In the words of (???), “Prolog is more versatile than SPARQL, allows composing of the logic from small building blocks and does not suffer from the object-relational impedance mismatch.”</p>
...
<p>In the case of Web Prolog, the need for a standard is there, not so much for the sake of portability, but more for the sake of interoperability – the ability of one implementation of Web Prolog to send code to as well as query other implementations of Web Prolog.</p>
...
<p>Web Prolog programs may be run also on machines that you don’t own or manage. This is similar to JavaScript, except that JavaScript (normally) runs on the client and Web Prolog runs on a server.</p>
<p>We don’t need to standardize the whole lot in one go. We could start with just a subset of ISO-Prolog and the NDRPC predicate. The Erlang-ish predicates can appear later.</p>
<hr>
...
<p>I’m interested in actually help realizing this thing. What I would do first is to find out if people such as Joe Armstrong and Robert Virding like the idea. And Richard O’Keefe should not be forgotten, he knows a lot about both languages.</p>
<p>I would then arrange a meeting.</p>
<p>I’m also willing to serve as the chair and editor of the standard. As mentioned above, I would like to appoint Jan Wielemaker dictator, at least as long as we are in the community group mode rather than in the working group mode.</p>
...
<p>BTW, don’t miss his talks, they are very enjoyable.</p>
<hr>
<p>Retrieve the text about purity by Markus<br>
dif should</p>
<p>You may regard it as my wishlist. I think the least we can do in return for Jan’s work is to have a wishlist.</p>
...
<p>Want to learn more about Bloom, Dedalus, and BOOM? Visit the project website, or check out Hellerstein’s keynote from PODS 2010 (includes synchronized video and slides; over an hour long).</p>
<p>Web Prolog can be parsed with a normal Prolog parser.</p>
<p>Learnability – is not going to lessen the difficulties with learning Prolog. But nor will it increase them. We are not looking for Learnability greater than Prolog. Web Prolog is not going to be easier than Prolog to learn, but not much harder either.</p>
<hr>
...
<hr>
<p>Consider two programmers A and B. They decide to divide a programming task between them. B Is assigned the task of developing a predicate p/1, to be used in A’s program. If A inserts the following clause in his program we have arrived at a simple way of doing collaborative programming:</p>
<hr>
<p>Pontus Wärnestål: Pro Bono Consultant – see his LinkedIn page</p>
<hr>
<pre><code>
p(X) :- p(X) @ 'http://b.org'.
...
<p>I’m addressing people much brighter than myself, while I’m only dabbling in a field that really isn’t my own.</p></div></body>
</html>